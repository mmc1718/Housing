...
TOKEN = "eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJkdGFibGVfdXVpZCI6IjljNDNhNTU4LWIzNmEtNGMyMy1iM2Y0LTQ0MThmMDRkY2QwZSIsImFwcF9uYW1lIjoiNzdXcC5weSIsImV4cCI6MTY1MDU2MDk3NX0.TItzHoJcoyMZ_mwcv-YVwc2vHmQgirTvqi-Q15KmF-w"
...
# This is a class for a Volunteer Table which is a child of Database
class VolunteerDatabase(Database):
    def __init__(self, base=Base):
        super().__init__(base, TableDef.VOLUNTEERS)
        super().setDummyRow({  
            "First Name": "",
            "Last Name": "",
            "QR": None,
...
def initStaticBase():
    ...
    volunteers = VolunteerDatabase(base)
    return volunteers     
...
def run(volunteers, var=None):

    url = "https://clipart.info/images/minicovers/1484599007rose-png-flower-646.png"
    ...
    info_dict = volunteers._base.upload_bytes_file('my_uploaded_file.png', image.content)
    img_url = info_dict.get('url')

    rows = volunteers.getAllRows()
    to_update = []
    for row in rows:
        if 'QR' not in row:
            unique_id = uuid.uuid4().hex
            print(unique_id)
            to_update.append({"row_id": row['_id'], "row": {"QR": [img_url], "Unique ID": unique_id}})


    volunteers._base.batch_update_rows(volunteers._tableName, to_update)
...
if MODE is DebugMode.EVENTBASED:
        volunteers = initSocketIOBase()
elif MODE is DebugMode.STATICLOCAL:
        volunteers = initStaticBase()
        run( volunteers) 
